---
title: "Replace `NA` with 7 and keep items with over 20% `NA` answers"
author: "Mingyang Cai"
date: ""
output: 
   html_document:
...
d50 <- d50 %>% 
  map(~ .x %>% 
        ...
        mutate_at(vars(!!!syms(paste0(qrp, "_raw"))), list(q = ~ recode(., `0` = 7))) %>% 
...
Model.2.modi <- '#latent variable definition
            QRP =~ QAttention + QSupervision + QResDesign + QUnfairRev 
                 + QUnsubstClaims + QCiteSource + QResNotes + QConcealDetails 
                 + QNoReSubmit + QConcealFlaws + QSelectCites 
                 + RR_sum + RCiteSource + ROpenCode + RErrorCheck
            RRP =~ RDisclFunds + RCorrectPub + RAuthorship + ROpenData 
                 + RKeepRecord + RPreReg + RDataManage + ROpenAccess 
                 + RCiteSource + ROpenCode + RErrorCheck + QNoReSubmit
...
The factor path plot shows the structure of the model with three factors. `MR2` is the RRP factor, `MR1` is one QRP factor (QRP.1) and `MR3` is the other QRP factor (QRP.2). In this case, efa loads QRPs into two separate QRP factors. 
...
Model.3 <- '#latent variable definition
            QRP.1 =~ QCiteSource + QUnfairRev + QSelectCites + QConcealDetails
                 + QUnsubstClaims + QConcealFlaws + QSupervision + QNoReSubmit 
                 + RR_sum
            
            QRP.2 =~ QResNotes + QAttention + QResDesign
            
            RRP =~ ROpenCode + ROpenData + RKeepRecord + RDataManage
                   + RErrorCheck + RCiteSource + RPreReg + RAuthorship
                   + ROpenAccess + RCorrectPub + RDisclFunds
...
Model.3.modi <- '#latent variable definition
            QRP.1 =~ QCiteSource + QUnfairRev + QSelectCites + QConcealDetails
                 + QUnsubstClaims + QConcealFlaws + QSupervision + QNoReSubmit 
                 + RR_sum + RCiteSource + ROpenCode
            
            QRP.2 =~ QResNotes + QAttention + QResDesign + RCiteSource
                   + ROpenCode + RErrorCheck
