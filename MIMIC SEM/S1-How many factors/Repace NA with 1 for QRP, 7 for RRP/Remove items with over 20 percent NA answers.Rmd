---
title: "Replace `NA` with `1` for QRP, `7` for RRP, and remove items with over 20% `NA` answers"
author: "Mingyang Cai"
date: ""
output: 
   html_document:
...
```
The CFI now is 0.97. I tried three factors model. However, the `fa` function in `psych` package faills to perform the factor analysis. The problem is the estiamted correlation matrix is non-positive and one possible solution is to increase the sample size. I skip to build the three factors model. I do not think it is a big issue since the two factors model with 5 modifications has a over 0.95 CFI.  
