---
title: "Replace `NA` with `0` and remove items with over 20% `NA` answers"
author: "Mingyang Cai"
date: ""
output: 
   html_document:
...
Model.3.modi <- '#latent variable definition
            QRP =~ QAttention + QSupervision + QResDesign + QUnfairRev 
                 + QUnsubstClaims + QCiteSource + QResNotes + QConcealDetails 
                 + QConcealFlaws + QSelectCites + RDisclFunds 
                 + RR_sum
            
            RRP.1 =~ RCiteSource + ROpenAccess + RDisclFunds + RAuthorship
                   + RErrorCheck + ROpenData + QUnfairRev + QConcealFlaws
                   + QCiteSource
