---
title: "Replace `NA` with `6` and keep items with over 20% `NA` answers"
author: "Mingyang Cai"
date: ""
output: 
   html_document:
...
d50 <- d50 %>% 
  map(~ .x %>% 
        mutate_at(vars(!!!syms(paste0(rrp, "_raw"))), list(r = ~ recode(., `0` = 6))) %>% 
        rename_at(vars(ends_with(("_raw_r"))), ~ gsub("_raw_r", "", .x)) %>% 
        mutate_at(vars(!!!syms(paste0(qrp, "_raw"))), list(q = ~ recode(., `0` = 6))) %>% 
...
Model.2.modi <- '#latent variable definition
            QRP =~ QAttention + QSupervision + QResDesign + QUnfairRev 
                 + QUnsubstClaims + QCiteSource + QResNotes + QConcealDetails 
                 + QNoReSubmit + QConcealFlaws + QSelectCites 
                 + RR_sum + ROpenCode + RPreReg + RCiteSource + RDisclFunds
            RRP =~ RDisclFunds + RCorrectPub + RAuthorship + ROpenData 
                 + RKeepRecord + RPreReg + RDataManage + ROpenAccess 
                 + RCiteSource + ROpenCode + RErrorCheck 
...
Model.3 <- '#latent variable definition
            QRP.1 =~ QCiteSource + QConcealDetails + QSelectCites + QUnfairRev
                   + QUnsubstClaims + RCiteSource + QConcealFlaws + QSupervision
                   + RDisclFunds + RAuthorship
                   + RR_sum
            QRP.2 =~ QResNotes + QAttention + QResDesign + QNoReSubmit
            RRP =~ ROpenCode + ROpenData + RKeepRecord + RDataManage
                 + RPreReg + RErrorCheck + ROpenAccess + RCorrectPub 
