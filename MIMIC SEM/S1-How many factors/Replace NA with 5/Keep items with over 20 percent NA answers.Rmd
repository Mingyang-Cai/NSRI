---
title: "Replace `NA` with `5` and keep items with over 20% `NA` answers"
author: "Mingyang Cai"
date: ""
output: 
   html_document:
...
d50 <- d50 %>% 
  map(~ .x %>% 
        mutate_at(vars(!!!syms(paste0(rrp, "_raw"))), list(r = ~ recode(., `0` = 5))) %>% 
        rename_at(vars(ends_with(("_raw_r"))), ~ gsub("_raw_r", "", .x)) %>% 
        mutate_at(vars(!!!syms(paste0(qrp, "_raw"))), list(q = ~ recode(., `0` = 5))) %>% 
...
The factor path plot shows the structure of the model with three factors. `MR1` is the QRP factor, `MR2` is one RRP factor (RRP.1) and `MR3` is the other RRP factor (RRP.2). In this plot, half of indicators load on the QRP factor.
...
Model.3 <- '#latent variable definition
            QRP =~ QAttention + QSupervision + QResDesign  
                 + QUnsubstClaims + QCiteSource + QConcealDetails + QUnfairRev 
                 + QNoReSubmit + QConcealFlaws + QSelectCites + RCiteSource
                 + RDisclFunds + RAuthorship
                 + RR_sum
            RRP.1 =~ RCorrectPub + ROpenAccess 
                   + ROpenCode + ROpenData + RPreReg
            RRP.2 =~ RKeepRecord + RDataManage       
                   + QResNotes + RErrorCheck
            #variance and covariance
            RDisclFunds ~~ RAuthorship
            RCorrectPub ~~ RErrorCheck
            ROpenData ~~ ROpenCode 
...
Model.3.modi <- '#latent variable definition
            QRP =~ QAttention + QSupervision + QResDesign  
                 ...
                 + QNoReSubmit + QConcealFlaws + QSelectCites + RCiteSource
                 + RDisclFunds + RAuthorship
                 + RR_sum + RKeepRecord + ROpenCode + RCorrectPub
            RRP.1 =~ RCorrectPub + ROpenAccess + RKeepRecord
                   + ROpenCode + ROpenData + RPreReg
            RRP.2 =~ RKeepRecord + RDataManage       
                   + QResNotes + RErrorCheck + ROpenCode
            #variance and covariance
            RDisclFunds ~~ RAuthorship
            RCorrectPub ~~ RErrorCheck
            ROpenData ~~ ROpenCode 
