...
We use the two factors model with 5 modifications which is replaced `NA` by `1` and kept items with over 20% NA answers. All 7-likert QRP and RRP items are treated as continuous variables in the invariance test. 
...
Model.2.2 <- cfa(Model.2.modi, data = dat, group = "Field",  group.equal = "loadings")
...
```
The CFI of the configural invariance model is 0.82. So I think the model does not fit well. Both p-values for model comparison tests are significant. Thus, we may conclude that weak invariance and strong invariance are not supported in this dataset.
