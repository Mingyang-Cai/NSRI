---
title: "Part 5-How many factors-15"
author: "Mingyang Cai"
date: ""
output: 
   html_document:
...
Replace 6 for the 0 scores (Not Applicable) in the raw RRP/QRP variables and remove 3 RRPs (`RCorrectPub`, `RPreReg` and `ROpenCode`) and 1 QRP (`QNoReSubmit`) whose percentages of `NA` answers exceed 20%. 
...
Model.2.modi <- '#latent variable definition
            QRP =~ QAttention + QSupervision + QResDesign + QUnfairRev 
                 + QUnsubstClaims + QCiteSource + QResNotes + QConcealDetails 
                 + QConcealFlaws + QSelectCites 
                 + RR_sum + ROpenData + RKeepRecord + RDisclFunds
            RRP =~ RDisclFunds + RAuthorship + ROpenData 
                 + RKeepRecord + RDataManage + ROpenAccess 
                 + RCiteSource + RErrorCheck 
