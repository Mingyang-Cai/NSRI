---
title: "Part 5-How many factors-12"
author: "Mingyang Cai"
date: ""
output: 
   html_document:
...
Replace 3 for the 0 scores (Not Applicable) in the raw RRP/QRP variables and remove 3 RRPs (`RCorrectPub`, `RPreReg` and `ROpenCode`) and 1 QRP (`QNoReSubmit`) whose percentages of `NA` answers exceed 20%. 
...
d50 <- d50 %>% 
  map(~ .x %>% 
        mutate_at(vars(!!!syms(paste0(rrp, "_raw"))), list(r = ~ recode(., `0` = 3))) %>% 
        rename_at(vars(ends_with(("_raw_r"))), ~ gsub("_raw_r", "", .x)) %>% 
        mutate_at(vars(!!!syms(paste0(qrp, "_raw"))), list(q = ~ recode(., `0` = 3))) %>% 
